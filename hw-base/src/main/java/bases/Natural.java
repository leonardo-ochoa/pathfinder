...
public class Natural {
  ...
  public static int leadingDigit(int[] digits) {
    ...
    // The loop invariant is saying that i + 1 must be zero inorder for it to hold.
    //{inv: D[i + 1] = 0, ..., D[n-1] = 0}
    while(i != 0 && digits[i] == 0 ){
      i = i - 1;
    }
    // At this point in the code, we know that i = 0 or digit[i] != 0.
    // This implies the post condition below, since digit[i] != 0 means D[i+1], ..., D[n-1] are all zero
    // or i == 0 .
  ...
  public Natural toBase(int base) {
    ...
    // here the post condition holds since this.value() = D[0] + D[1] b + D[2] b^2 + ... + D[n-1] b^{n-1}
  ...
  public String toString() {
    ...
    // This implies the post condition below, since we have appended the digit at the starting.
    // i all the way down to 0.
  ...
  public Natural plus(Natural other) {
    ...
    int i = 0;  //
    // at each index starting from 0 up to B's length, D will have the sums of A and B
    ...
    //Since A or B could have had at most b - 1, adding their maximum digits would be the same as multiplying by 2,
    // which would get at most 2b - 2 which is less than 2b thus the above hols for digit values 0 to n - 1.

    // it holds initially because we have already added the digits of both A and B into D where we stopped
    // a B lengths.

    // The invariant below tells us that we will need to add the digits of A and B into D.
    // If the i value is left than the length of B then we will add the digits of B  and A into D otherwise
    // we only add the digits of A into D.
    // Inv: D[0] = A[0]+B[0], D[1] = A[1]+B[1], ..., D[i-1] = A[i-1]+B[i-1],
...
//      return null;
    ...
    // Since both this and other are of the same base, adding their digits get us a new value, which if we were to call
    // getValue(), it would be the same as this.value + other.value.
    ...
    // The post condition holds because we have what got returned was the values of this and other which
    // is D[0] + D[1]b + ... + D[n-1] b^{n-1}. It also holds since all the Digits up to index i - 1 are less than
    // the base.
    //                   (2) the preconditions of this constructor hold.
    // The precondition for the constructor holds since the newDigits that got passed in has values
    // that are between 0 and this.base - 1.
